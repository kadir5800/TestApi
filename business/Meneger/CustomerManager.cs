...
namespace Business.Meneger
{
    public class CustomerManager : ZServerService, ICustomerManager
    {
        ...
        public ClientResult<getAllCustomerResponse> getAllCustomer(getAllCustomerRequest request)
        {
            ...
            {
                case "Addres":
                    customerList = sortDirection == "asc" ? customerList.OrderBy(c => c.Addres).ToList() : customerList.OrderByDescending(c => c.Addres).ToList();
                    break;
                case "CreationDate":
                    customerList = sortDirection == "asc" ? customerList.OrderBy(c => c.CreationDate).ToList() : customerList.OrderByDescending(c => c.CreationDate).ToList();
                    break;
                case "Email":
                    customerList = sortDirection == "asc" ? customerList.OrderBy(c => c.Email).ToList() : customerList.OrderByDescending(c => c.Email).ToList();
                    break;
                case "Name":
                    customerList = sortDirection == "asc" ? customerList.OrderBy(c => c.Name).ToList() : customerList.OrderByDescending(c => c.Name).ToList();
                    break;
                case "Phone":
                    customerList = sortDirection == "asc" ? customerList.OrderBy(c => c.Phone).ToList() : customerList.OrderByDescending(c => c.Phone).ToList();
                    break;
                case "Phone2":
                    customerList = sortDirection == "asc" ? customerList.OrderBy(c => c.Phone2).ToList() : customerList.OrderByDescending(c => c.Phone2).ToList();
                    break;
                case "Surname":
                    customerList = sortDirection == "asc" ? customerList.OrderBy(c => c.Surname).ToList() : customerList.OrderByDescending(c => c.Surname).ToList();
                    break;
            }
